...
class NarrateController extends BaseController
{
    public function generate()
    {
        ...
        $voiceId = $this->request->getPost('voice_id');
        ...
        // Atualiza vídeo se video_id informado
        if ($videoId) {
            db_connect()->table('videos')->where('id', $videoId)
                ->update(['voice_id' => $voice['id'], 'audio_path' => $result['rel_path'], 'status' => 'ready']);
        }
