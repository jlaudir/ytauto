...
class PlanModel extends Model
{
    ...
    public function getWithPermissions(int $id): array
    {
        ...
        $perms = $db->table('plan_permissions pp')
            ->select('permissions.*')            
    ...
    public function getStats(): array
    {
        ...
        return $db->table('plans p')
            ->select('p.id, p.name, p.price_monthly,p.is_active,p.price_annual,p.max_videos_month, COUNT(DISTINCT u.id) as user_count, COUNT(DISTINCT s.id) as active_subs')            
