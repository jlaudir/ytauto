...
// ── Pagamentos / PIX ──────────────────────────────────────────
$routes->post('payment/asaas/pix',       'Payment\AsaasController::generatePix',  ['filter' => 'client']);
$routes->get('payment/asaas/status/(:num)', 'Payment\AsaasController::checkStatus/$1', ['filter' => 'client']);
$routes->post('payment/asaas/webhook',   'Payment\AsaasController::webhook');   // sem filtro (Asaas chama direto)
...
$routes->group('app', ['filter' => 'client'], function($routes) {
    ...
    $routes->get('upgrade',         'Client\Profile::upgrade');
    $routes->post('upgrade',        'Client\Profile::doUpgrade');
...
$routes->group('admin', ['filter' => 'admin'], function($routes) {
    ...
    $routes->post('settings/test-asaas', 'Admin\Settings::testAsaas');
